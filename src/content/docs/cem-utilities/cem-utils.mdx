...
```ts
type getAllComponents<T extends Component>(
...
```ts
type getComponentByTag<T extends Component>(
  customElementsManifest: unknown,
  tagName: string
) => T;
```
...
```ts
type getComponentByClassName<T extends Component>(
  customElementsManifest: unknown,
  className: string
) => T;
```
...
```ts
type getComponentPublicProperties<T extends Property>(
  component: Component
) => T[];
```
...
```ts
type getComponentPublicMethods<T extends Method>(
  component: Component
) => T[];
```
...
```ts
type getComponentEventsWithType<T extends ComponentEvent>(
  component: Component,
