...
The helpers include a reloader that watches your Custom Elements Manifest (`custom-elements.json`) and forces Storybook to reload the iframe when the manifest changes. This is enabled by default.
...
To customize the manifest path or disable the feature:
