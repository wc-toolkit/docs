...
If you are using third-party libraries that have class names that conflict with your own component class names, you can use the `aliasMap` option to map class names to alternative names.

This is useful when you want to avoid naming conflicts and ensure that the inheritance mapping works correctly.

```ts
// my-button.js
import { Button as NewButton } from 'third-party-library';

export class Button extends NewButton {
